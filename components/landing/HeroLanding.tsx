...
function SectionHeading({ tone, children }: { tone: Tone; children: ReactNode }) {
  return (
    <div className="flex items-center gap-3">
      <span
        ...
        aria-hidden
      />
      <h2 className="font-display text-2xl font-bold sm:text-3xl" style={{ color: INK }}>
        {children}
      </h2>
    </div>
  );
...
export function HeroLanding({
  ...
  return (
    <div className="h-full w-full overflow-x-hidden overflow-y-auto bg-white">
      ...
      >
        ...
        <div className="relative z-10 flex max-w-2xl flex-col items-center">
          ...
          <button
            type="button"
            onClick={onEnter}
            style={{ backgroundColor: EMBER, boxShadow: "0 12px 28px -8px rgba(232,99,44,0.55)" }}
        ...
        <ol className="mt-8 grid gap-6 sm:grid-cols-3">
          {steps.map((step, index) => (
            <li key={step.title} className="flex flex-col gap-3">
              ...
              <h3 className="font-display text-base font-semibold" style={{ color: INK }}>
                {step.title}
              </h3>
      ...
      <section className="border-y" style={{ backgroundColor: ICE_BG, borderColor: LINE }}>
        <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
          <SectionHeading tone="ember">{t.rulesTitle}</SectionHeading>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed" style={{ color: INK_MUTED }}>
            {t.rulesIntro}
          </p>
      ...
      {site && (
        <section className="border-t-[3px]" style={{ backgroundColor: ICE_BG, borderColor: FROST }}>
