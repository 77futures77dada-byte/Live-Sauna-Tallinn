...
5. **Booking без оплаты = booking без гарантии, зафиксировано QR-явкой** (2026-08-12) — раз оплаты в MVP нет, "бронь" сама по себе не держит место: `bookings.status` становится `'fulfilled'` только когда посетитель делает check-in через уже существующий QR-flow (`app/api/visits`) в течение забронированного часа, а `booking_id` линкуется к созданному `visit_id`. Не явившиеся к концу часа брони помечаются `'no_show'` (ленивая проверка при следующем чтении, отдельный воркер не нужен для MVP). ~~Анкета бронирования требует verification-фото (`photos.type = 'booking_verification'`, `photos.booking_id`) до создания брони.~~ **Отменено, см. пункт 6.** Применяется одинаково к Harku и Iglupark Noblessner — единственным двум локациям с `booking_enabled = true` на Phase 4a (миграция `0007_booking_qr_confirmation.sql`). Harku заодно меняет `type` с `sauna_swimming` на `sauna` (capacity 8 без изменений).
6. **Verification-фото при самой брони убрано** (2026-08-25) — пункт 5 был ошибочным: бронь оформляется удалённо, до физического визита, так что "фото на месте" на этапе бронирования физически невозможно взять. Явка теперь полностью покрывается check-in flow'ом: обязательные геолокация + before-фото при "I'm here" (см. `lib/geo.ts`, `CHECKIN_RADIUS_METERS`), плюс уже существующее after-фото при завершении визита (Phase 3). `POST /api/bookings` больше не принимает файл и не создаёт строки `photos.type = 'booking_verification'` — тип остаётся в схеме (`0007_booking_qr_confirmation.sql`, `PhotoType` в `lib/supabase/types.ts`) неиспользуемым, без отдельной миграции на удаление. `booking → visit → 'fulfilled'` через geo+photo check-in остаётся единственным и главным механизмом подтверждения явки, без изменений.
